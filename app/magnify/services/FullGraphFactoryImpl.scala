...
import magnify.features.{FullGraphFactory, FullGraphMetric, MetricsProvider, RevisionGraphFactory}
...
class FullGraphFactoryImpl(
    ...
    metrics: MetricsProvider[FullGraph, FullGraph, FullGraphMetric]) extends FullGraphFactory {
  ...
  def load(fileName: String): FullGraph = {
    ...
    // not all metrics are serializable
    val graphWithMetrics = metrics().filterNot(_.isSerializable).foldLeft(graph) { case (graph, metric) =>
      metric(graph)
    }
    Future {
